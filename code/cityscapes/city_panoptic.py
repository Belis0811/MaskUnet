...
# -------------------------------
# Utility function: generate instance mask from semantic mask
# -------------------------------
...
class CityscapesPanopticDataset(Dataset):
    def __init__(self, img_dir, ann_dir, transforms=None, img_size=(128, 128)):
        ...
        # List all image files recursively from the leftImg8bit folder.
    ...
    def __getitem__(self, idx):
        ...
        image_id = base.split("_leftImg8bit")[0]

        # Build the semantic annotation filename using a wildcard pattern.
        sem_pattern = os.path.join(self.ann_dir, city, f"{image_id}*gtFine_labelIds.png")
        sem_files = glob.glob(sem_pattern)
        if len(sem_files) == 0:
            raise FileNotFoundError(f"Could not find semantic annotation matching {sem_pattern}")
        sem_path = sem_files[0]
        semantic_mask = cv2.imread(sem_path, cv2.IMREAD_GRAYSCALE)
        semantic_mask = np.where((semantic_mask >= 0) & (semantic_mask < 19), semantic_mask, 255)
        if semantic_mask is None:
            raise FileNotFoundError(f"Could not load semantic annotation from {sem_path}")

        # Build the instance annotation filename using a wildcard pattern.
        inst_pattern = os.path.join(self.ann_dir, city, f"{image_id}*gtFine_instanceIds.png")
        inst_files = glob.glob(inst_pattern)
        if len(inst_files) == 0:
            raise FileNotFoundError(f"Could not find instance annotation matching {inst_pattern}")
        inst_path = inst_files[0]
        instance_mask = cv2.imread(inst_path, cv2.IMREAD_UNCHANGED)
        if instance_mask is None:
            raise FileNotFoundError(f"Could not load instance annotation from {inst_path}")
...
# -------------------------------
train_dataset = CityscapesPanopticDataset(
    img_dir=IMG_DIR_TRAIN,
    ann_dir=ANN_DIR_TRAIN,
    transforms=ToTensor(),
    img_size=(IMG_HEIGHT, IMG_WIDTH)
)
val_dataset = CityscapesPanopticDataset(
    img_dir=IMG_DIR_VAL,
    ann_dir=ANN_DIR_VAL,
    transforms=ToTensor(),
    img_size=(IMG_HEIGHT, IMG_WIDTH)
...
# -------------------------------
# Model definition (same UNet architecture)
# -------------------------------
...
# -------------------------------
# Early Stopping and Instance Contrastive Loss
# -------------------------------
class EarlyStopping:
...
#     checkpoint = torch.load("checkpoint_city_pan.pth")
...
best_loss = float("inf")
best_iou = 0.0
log_file = open("training_log_city_pan.txt", "w")
for epoch in range(num_epochs):
    ...
    for i, (inputs, sem_labels, inst_labels) in enumerate(tqdm(train_loader)):
        ...
        if i % 500 == 0 and i != 0:
            ...
            if i % 2000 == 0:
                print("Saving model...")
                torch.save(model.state_dict(), "checkpoint_city_pan.pth")
    ...
    if best_iou - avg_iou > 0.1:
        print("Model Overfit")
        break
    torch.save(model.state_dict(), "epoch_city_pan.pth")
...
best_val_iou = 0.0
log_file = open("validating_log_city_pan.txt", "w")
model.eval()
for epoch in range(num_epochs):
    total_val_loss = 0.0
    total_val_iou = 0.0
