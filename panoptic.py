...
def evaluate_panoptic(model, val_loader, dataset, device):
    ...
    with torch.no_grad():
        for images, sem_masks, inst_masks in tqdm(val_loader):
            ...
            for i in range(len(images)):
                inst_preds = generate_instance_mask(sem_preds[i])

                pred = {
                    "image_id": dataset.images[i]["id"],
                    "file_name": f"pred_{dataset.images[i]['file_name']}",
                    "segments_info": [],  # Placeholder, to be filled
                }
                ...
                gt = {
                    "image_id": dataset.images[i]["id"],
                    "file_name": dataset.annotations[dataset.images[i]["id"]]["file_name"],
                    "segments_info": dataset.annotations[dataset.images[i]["id"]]["segments_info"]
                }
    ...
    gt_file = dataset.panoptic_json  # Ground-truth annotation file
    pq_results = pq_compute(gt_json_file=gt_file, pred_json_file=pred_file)
...
for epoch in range(0,num_epochs):
    ...
    if epoch % 5 == 0 and epoch != 0:
        pq_results = evaluate_panoptic(model, val_loader, val_dataset, device1)
        ap = compute_ap(val_dataset.panoptic_json, "predictions.json")
